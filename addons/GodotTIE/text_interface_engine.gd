...
# Mes ajouts
onready var _sound_manager = AudioStreamPlayer2D.new()
...
func buff_text(text, vel = 0, tag = "", push_front = false, sound=null): # The text for the output, and its printing velocity (per character)
	var b = {"buff_type":BUFF_TEXT, "buff_text":text, "buff_vel":vel, "buff_tag":tag, "buff_sound":sound}
	if !push_front:
		_buffer.append(b)
		_sound_manager.set_stream(_buffer[_buffer.size()-1]["buff_sound"])
	else:
		_buffer.push_front(b)
		_sound_manager.set_stream(_buffer[0]["buff_sound"])
...
func _ready():
	...
	_sound_manager.set_volume_db(0)
	_sound_manager.set_bus("Master")
	_sound_manager.set_area_mask(1)
	_sound_manager.set_max_distance(500)
	_sound_manager.set_attenuation(1)
